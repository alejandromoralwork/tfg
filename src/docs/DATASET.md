...
> This file documents the **upstream dataset** itself (Albers, Cucuringu, Howison &amp;
> Shestopaloff, 2026) — its archives, sizes, and the reference Python reader it ships with.
> It is not documentation of `market_sim`'s own code. For the crate's binary decoder
> specifically, see [`SCHEMA.md`](SCHEMA.md) (the byte-level record spec `market_sim` decodes
> against) and [`REFERENCE.md`](REFERENCE.md#binary_formatrs) (`inputs/binary_format.rs`). See
